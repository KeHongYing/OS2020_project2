...
if __name__ == '__main__':
	...
	print('master_average_filesize(kbytes/ms):', np.mean(master_filesizes) / np.mean(master_times) / 1024)
	...
	print('slave_average_filesize(kbytes/ms):', np.mean(slave_filesizes) / np.mean(slave_times) / 1024)
	print("=======================================================================")
	print(f"| {args.master_type} | {args.slave_type} | {np.round(np.mean(slave_times), 3)} | {np.round(np.std(slave_times), 3)} | {np.round(np.mean(slave_filesizes) / np.mean(slave_times) / 1024, 3)} |")
