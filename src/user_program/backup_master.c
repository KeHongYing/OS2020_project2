...
int main (int argc, char* argv[])
{
	...
	for(int i = 0; i < n; i++) {
		...
		//sprintf(buf, "%lld", file_size);
		//write(dev_fd, buf, sizeof(strlen(buf)));//write file size to the the device
		write(dev_fd, &file_size, sizeof(file_size));//write file size to the the device
